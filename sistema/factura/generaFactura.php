...
if(empty($_REQUEST['cl']) || empty($_REQUEST['f'])) {
	...
	$productos = mysqli_query($conexion, "SELECT d.nofactura, d.codproducto, d.cantidad, p.codproducto, p.descripcion, p.precio 
		FROM detallefactura d 
		INNER JOIN producto p ON d.codproducto = p.codproducto 
		WHERE d.nofactura = $noFactura");

	// Inicializar los totales
	$subtotal = 0;
	$ivaPorcentaje = $resultado['igv'];
	$iva = 0;
	$total = 0;

	// Configuración para descarga en PDF
	...
	$pdf->Cell(76, 5, 'IGV (' . $ivaPorcentaje . '%): ' . number_format($iva, 2, '.', ','), 0, 1, 'R');
	...
	$pdf->Ln();
	$pdf->SetFont('Arial', '', 7);
	$pdf->Cell(80, 5, utf8_decode("Gracias por tu compra, Dios te bendiga"), 0, 1, 'C');
