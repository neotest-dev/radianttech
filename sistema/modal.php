  ...
  if (empty($_POST['producto']) || empty($_POST['cantidad'])){
    ...
    if ($result > 0) {
      if ($result_iva > 0) {
        ...
        $iva = $info_iva['igv']; // IGV en porcentaje, por ejemplo, 18
      }
      
      while ($data = mysqli_fetch_assoc($query_detalle_temp)) {
        $precioTotal = round($data['cantidad'] * $data['precio_venta'], 2);
        $sub_total += $precioTotal; // Sumando todos los precios
        $detalleTabla .= '<tr>
      ...
      $impuesto = round($sub_total * ($iva / 100), 2);  // Calcular IGV sobre el subtotal
      $total = round($sub_total + $impuesto, 2);  // Total con IGV
      
      // Formatear los valores con dos decimales
      $detalleTotales = '<tr>
          <td colspan="5" class="textright">Sub_Total S/.</td>
          <td class="textright">'.number_format($sub_total, 2).'</td>
      </tr>
      <tr>
          <td colspan="5" class="textright">IGV ('.$iva.'%)</td>
          <td class="textright">'.number_format($impuesto, 2).'</td>
      </tr>
      <tr>
